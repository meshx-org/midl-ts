...
class BaseMessage {
    ...
    hexDump(): void {
        const width = 16
        let list = new Uint8Array(this.data.buffer, 0)
        let buffer: string[] = []
...
function validateDecoding(decoder: Decoder): void {
    ...
    if (decoder.countUnclaimedHandles() > 0) {
        ...
        let unclaimed = decoder.countUnclaimedHandles()
        let total = decoder.handleInfos.length
...
export function decodeMessageWithCallback<T>(message: IncomingMessage, inlineSize: number, f: DecodeMessageCallback<T>): T {
    ...
    let out = f(decoder, kMessageHeaderSize)
